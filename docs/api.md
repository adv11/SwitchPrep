...
| `toggleFavoriteRoadmap` | `async (id: string) => { ok: boolean, capped: boolean }` | Issue #177 — adds `id` to `favoriteRoadmapIds` if under `MAX_FAVORITE_ROADMAPS` (3, also exported), removes it if already present, and persists to `users/{uid}/meta/favoriteRoadmapIds` (plus a local fallback). Returns `{ ok: false, capped: true }` without mutating anything if adding a 4th would exceed the cap — callers must check this. No distinction between a built-in template id and a `croadmap-...` custom id. |
...
```ts
{
  ...
  favoriteRoadmapIds: string[], // issue #177 — up to MAX_FAVORITE_ROADMAPS (3) roadmap ids,
...
|---|---|---|---|
| `computeAnalytics` | `analyticsEngine.js` | `(items, activityLog, now?, streakFreezes?) => { overview, streaks, streakFreezesAvailable, velocity, phaseBreakdown, priorityBreakdown, heatmapData, projection }` | Composes every sub-metric below. Internally merges `activityLog` with a backfill derived from items' `completedAt` (see `buildEffectiveActivityLog`) before computing anything date-based. `streakFreezes` (issue #179, default `{ available: 0, usedDates: [] }`) is `activityLogStore.getSnapshot().streakFreezes` — its `usedDates` feeds `computeStreaks`, its `available` passes straight through as `streakFreezesAvailable`. |
...
| `computeStreaks` | `streaks.js` | `(activityLog, now?, frozenDates?) => { current, longest }` | A day "counts" if its entry is `>= 1`, or its dateKey is in `frozenDates` (issue #179 — a spent streak-freeze token). Today with 0 activity doesn't break the current streak, just doesn't extend it. `longest` is never affected by freezes. |
