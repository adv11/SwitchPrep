...
| Export | Signature | Notes |
|---|---|---|
| `authApi` | `signIn, signUp, guest, signInWithGoogle, signOut, linkGuest, sendResetEmail, sendVerificationEmail, setPersistence, deleteAccount, onChange` | Thin wrappers around Firebase Auth. `signInWithGoogle()` (issue #5 part 3, revised as a follow-up) requests the Drive `drive.appdata` scope directly on its `GoogleAuthProvider` and resolves `{ user, driveAccessToken }` — one popup covers both identity and Drive consent. Callers pass `driveAccessToken` to `googleDriveAuth.js`'s `setInitialAccessToken()` — see below. |
...
| Export | Signature | Notes |
|---|---|---|
| `requestInitialToken` | `() => Promise<string>` | Shows GIS's own popup/consent for `drive.appdata` scope, independent of any Firebase identity flow. No longer called by the normal "Sign in with Google" button (see `setInitialAccessToken` below) — kept for a future "Connect Google Drive" flow on an existing Firebase account, which has no Firebase popup to piggyback a scope onto. Rejects if the grant is denied/closed. |
| `setInitialAccessToken` | `(token: string) => void` | Seeds the in-memory token from the OAuth credential `authApi.signInWithGoogle()` already returns, and arms the same silent-refresh timer `requestInitialToken()` would have. Called from `signIn.js` right after `signInWithGoogle()` resolves — this is what lets the normal sign-in flow use a single popup instead of a separate GIS grant. Assumes a 1-hour token lifetime (Firebase's credential doesn't expose the real `expires_in`). |
