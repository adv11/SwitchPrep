...
**That fourth call site showed up — twice — and neither was routed through the queue, issue #350.** `toggleFavoriteRoadmap()` (favoriting/unfavoriting `favoriteRoadmapIds`, the primary everyday way a user favorites a roadmap via the onboarding overflow menu) and `setHiddenTemplateIds()` (backing `hideTemplate()`/`unhideTemplate()`, mutating `hiddenTemplateIds`) both called `adapter.saveMeta()` directly, computing their next array from the shared variable before any `await` — the exact pre-#153 shape. Two overlapping favorites (or two overlapping hides) for different ids reproduced the identical lost-update mechanism: whichever `saveMeta()` write landed last silently erased the other's change. Both are now routed through `serializeMetaMutation()`: `toggleFavoriteRoadmap(id)` keeps its `MAX_FAVORITE_ROADMAPS` cap check outside the queue (a synchronous read of current in-memory state, no race risk, same precedent as `switchRoadmap()`'s `alreadyStarted` snapshot) but recomputes `isFavorite`/the next array *inside* the queued closure. `setHiddenTemplateIds()` was reshaped to take a `computeNextIds(currentIds) => nextIds` function rather than a pre-computed array — `hideTemplate()`/`unhideTemplate()` used to build the next array themselves before calling it, which would have left the same race in place even with the function itself wrapped in the queue, since the array was already stale by the time it arrived. See the "lost-update race on favoriteRoadmapIds/hiddenTemplateIds" describe block in `tests/integration/roadmapStore.test.js`, same fails-before/passes-after verification discipline as issue #153's own tests. If you add a fifth array-field mutation, route it through `serializeMetaMutation()` and make sure the *computation*, not just the save, happens inside the queued closure — wrapping the closure around an already-computed value doesn't fix anything.

**A third, previously unlisted call site with the exact same shape: `shareStore.js`'s `shareIds`, issue #375.** `publishRoadmapShare()`/`revokeRoadmapShare()` (see "Roadmap sharing" below) do their own read-modify-write on `users/{uid}/meta/shareIds` — a plain array field, same shape as `startedTemplateIds`/`favoriteRoadmapIds`/`hiddenTemplateIds` above, but living in a separate module that was never audited alongside those three because `shareStore.js` was deliberately kept independent of `roadmapStore.js`'s `StorageAdapter` machinery (see the "Roadmap sharing" section's own note on why). It was never routed through `serializeMetaMutation()` when built (issue #131) or when the #153 fix landed later, since #153 predates #131's `shareIds` field entirely. `roadmapStore.js`'s `serializeMetaMutation()` isn't importable from `shareStore.js` — it's created fresh inside `createRoadmapStore()`'s closure, not exported at module scope — so the fix is a dedicated equivalent queue local to `shareStore.js` (`serializeShareIdsMutation()`), same discipline: the next `shareIds` array is computed inside the queued closure, never before it. **If you find a fourth call site outside this file's own `serializeMetaMutation()`-covered set, check every module that reads-then-writes any per-user array field under `users/{uid}/meta`, not just `roadmapStore.js` itself** — this file only enforces the queue for mutations it owns directly.
...
**Two Firebase paths, one `update()` call.** `submitReport()` generates a push key under `reports/` and writes the identical payload to both `reports/{reportId}` (developer-only, `.read: false`, reviewed via the Firebase console) and `users/{uid}/reports/{reportId}` (the user's own "My reports" history) in a single multi-path `update(ref(database), { [...]: ..., [...]: ... })`, so a client never observes one path written without the other. Before issue #348 removed the screenshot feature, the two paths carried different shapes (the top-level path included `screenshotB64`, the per-user mirror stripped it via `buildReportSummary()`) — with no screenshot field left to strip, both paths get the same `buildReportPayload()` output and `buildReportSummary()` was deleted as dead code. `firebase/database.rules.json`'s `reports/{reportId}` validate rule requires `userId === auth.uid`, so a client can't forge a report under someone else's identity even though the path itself isn't uid-keyed.
...
**`sharedRoadmaps/{shareId}` was missing its own `$other` catch-all, and `phases` had zero `.validate` rule at all — a critical gap on the app's one public-read path, closed in issue #351.** Every sibling object-shaped subtree (`roadmaps/{templateId}`, `meta`, `dailyTodos/{todoId}`, `streakFreezes`, even `sharedRoadmaps/{shareId}/items/{itemId}` one level deeper) already had a `$other: { ".validate": "false" }` catch-all per issue #275's sweep — `sharedRoadmaps/{shareId}` itself, the object one level *up* from `items`, was the one place that sweep never reached. Combined with `phases` (written on every `buildRoadmapShareSnapshot()` publish, `src/core/roadmap/shareSchema.js`) having no shape/size check whatsoever, any authenticated user (including a guest — anonymous auth still gets a real uid) could publish a share and then send a follow-up write adding arbitrary unbounded extra top-level fields, or an unbounded/malformed `phases` array, to the one world-readable path in the app. `phases` now mirrors the private `roadmaps/{templateId}/phases` schema (`id`/`title`/`priority`/`resourceKey`/`sections`), but with **tighter** index caps than the private path (`$index.matches(/^[0-9]{1,2}$/)`, 100 phases/100 sections each vs. the private path's 1,000) — since this data is public regardless of size, err toward the same-or-tighter cap, never a looser one. The `$other` catch-all now sits alongside `schemaVersion`/`ownerUid`/`templateId`/`title`/`publishedAt`/`items`/`phases`. Same root cause, lower urgency (both are uid-scoped, no cross-user leak): `users/{uid}/roadmap` (the legacy singular path) and both `reports/{reportId}` paths (top-level and the per-user summary mirror) were also missing this catch-all — fixed in the same PR, with explicit permissive field-level rules added for every field each path's real payload actually writes (a `$other: false` added without first covering every legitimately-written field breaks every real write outright, since an unlisted field becomes indistinguishable from a malicious one — this is exactly why `phases`' rule had to land in the same change as its `$other`, not before it). Verified against a real local Firebase emulator (`npx firebase emulators:start --only database`) — see `tests/e2e/dataCapRules.test.js`'s `sharedRoadmaps/{shareId}` case for the extra-field/malformed-phases/compliant-publish coverage.
